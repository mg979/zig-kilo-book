...
with their flags, keywords and so on. Create a module named `syndefs.zig` and
copy-paste them.

You might note that all arrays have a `&` in front of them. That's because the
`Syntax` type's fields are _slices_, for example `[]const []const u8`.
The only exception is `mlcmt`, because it has a fixed size, besides being
optional.
